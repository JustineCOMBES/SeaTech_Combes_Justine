...
PTPER = 100*PWMPER;           //Periode en pourcentage
...
FCLCON1 = 0x0003 ;            //Desactive la gestion desfaults
...
FCLCON6 = 0x0003 ;            //Desactive la gestion desfaults
/*Enable PWM Module */
PTCONbits.PTEN = 1;
}
...
void PWMSetSpeed( float vitesseEnPourcents, int motor)
{
    if(motor == MOTEUR_GAUCHE){
        ...
        if(vitesseEnPourcents<0){ // reculer
        ...
        if(vitesseEnPourcents>=0){ // avancer
...
void PWMUpdateSpeed() {
    ...
    if (robotState.vitesseGaucheCommandeCourante > 0) {
        MOTEUR_GAUCHE_L_PWM_ENABLE = 0; // p i l ot a ge de la pin en mode IO
        MOTEUR_GAUCHE_L_IO_OUTPUT = 1; //Mise à 1 de la pin
        MOTEUR_GAUCHE_H_PWM_ENABLE = 1; // Pilotage de la pin en mode PWM
    } else {
        MOTEUR_GAUCHE_H_PWM_ENABLE = 0; // pi l o ta g e de la pin en mode IO
        MOTEUR_GAUCHE_H_IO_OUTPUT = 1; //Mise à 1 de la pin
        MOTEUR_GAUCHE_L_PWM_ENABLE = 1; // Pilotage de la pin en mode PWM
    }
    MOTEUR_GAUCHE_DUTY_CYCLE = Abs(robotState.vitesseGaucheCommandeCourante) * PWMPER;
...
void PWMSetSpeedConsigne(float vitesseEnPourcents, int motor) {
    if (motor == MOTEUR_GAUCHE) {
